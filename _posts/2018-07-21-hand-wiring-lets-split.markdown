...
would definetley recommend the project to others. Me and some friends at the
...
wiring matrix to your micro controller, and how to set up the software config
to use the correct pins. So I will try to expand on that here, standing on the
shoulders of giants. The text should be useable for anyone hand wiring a
...
[Picture courtsey of Adafruit](https://learn.sparkfun.com/tutorials/pro-micro--fio-v3-hookup-guide/hardware-overview-pro-micro "Adafruits website")
...
What you need to do is making sure that there is connecin between each row and
...
anywere on the long wire running on the
...
cols it's even simpler. Litterally anywhere on the wire or on any switch pin is
...
Nothing difficult at all, but it had me worried for a while and I've recieved
questions about it so I figuered I might as well try to explain it.

Deciding on where to attach the wires (to the grid and to the micro controller)
...
This is probably what is the most scary part for anyone doing their first
...
there was only one extra step. You need to let the micro computer know which
